...
void credits_tick_particles(void)
{
	...
	if (g_CreditsData->particlecolourindex1 >= 0) {
		...
		if (RANDOMFRAC() < 0.007f && joy_get_buttons(0, R_TRIG) == 0) {
...
#if VERSION >= VERSION_NTSC_1_0
	if (RANDOMFRAC() < 0.002f && joy_get_buttons(0, R_TRIG) == 0) {
...
void credits_tick(void)
{
	...
	if (g_CreditsData->slidesenabled) {
		credits_tick_slide();
	} else if (RANDOMFRAC() < 0.01f && !joy_get_buttons(0, R_TRIG)) {
		credits_create_pending_bg_layers(0xffffffff);
	}
...
Gfx *credits_draw(Gfx *gdl)
{
	...
	if (joy_get_buttons_pressed_this_frame(0, 0xffcf) ||
		inputKeyJustPressed(VK_ESCAPE))
#else
	if (joy_get_buttons(0, 0xffff) ||
		inputKeyJustPressed(VK_ESCAPE))
#endif
#endif
	{
