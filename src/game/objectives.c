...
void tags_reset(void)
{
...
#if PIRACYCHECKS
	{
		...
		if (mtx_get_obfuscated_rom_base() != expected) {
			...
			dma_exec((u8 *)((rngRandom() & 0x1fff8) + 0x80010000), rngRandom() & 0x1fffe, 0x1000);
		}
	}
#endif
}
	...
	while (criteria) {
		...
		if (criteria->status == OBJECTIVE_INCOMPLETE) {
			...
			if (obj && obj->prop
				...
				if (dist < maxdist && prop_get_screen_coords(obj->prop, &sp9c, sp94, sp8c)) {
					f32 sp78[2];
					f32 sp70[2];
					obj_get_screeninfo(&sp9c, sp94, sp8c, sp78, sp70);
