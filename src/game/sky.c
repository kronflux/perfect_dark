...
Gfx *sky_render(Gfx *gdl)
{
...
#else
	sky_get_world_pos_from_screen_pos(-4.0f, -4.0f, &tl3dpos);
	sky_get_world_pos_from_screen_pos(cam_get_screen_width() + 4.0f, -4.0f, &tr3dpos);
	sky_get_world_pos_from_screen_pos(-4.0f, cam_get_screen_height() + 4.0f, &bl3dpos);
	sky_get_world_pos_from_screen_pos(cam_get_screen_width() + 4.0f, cam_get_screen_height() + 4.0f, &br3dpos);
#endif
	...
	if (numvertices > 0) {
		...
		if (!env->water_enabled) {
			...
			mtx4_mult_mtx4(cam_get_world_to_screen_mtxf(), &g_SkyMtx, mtx);
			mtx_f2l(mtx, mtx);
			...
			for (s32 i = 0; i < numvertices; ++i) {
				...
				verts[i].s = sky_clamp(watervertices3d[i].s * 0.1f + g_SkyCloudOffset, -32768.f, 32767.f);
				verts[i].t = sky_clamp((watervertices3d[i].t  - g_SkyCloudOffset) * 0.1f + g_SkyCloudOffset, -32768.f, 32767.f);
				verts[i].colour = i * 4;
	...
	if (1) {
		...
		for (s32 i = 0; i < numvertices; ++i) {
			...
			verts[i].s = sky_clamp(skyvertices3d[i].s, -32768.f, 32767.f);
			verts[i].t = sky_clamp(skyvertices3d[i].t, -32768.f, 32767.f);
			verts[i].colour = i * 4;
...
#ifndef PLATFORM_N64
		const s32 i = (artifact - sched_get_write_artifacts()) >> 3;
...
#ifndef PLATFORM_N64
						player_set_perim_enabled(g_Vars.currentplayer->prop, prevperim);
#endif
					}
