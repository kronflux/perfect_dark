...
{
	return sight_draw_classic(gdl, sighton, crossx, crossy);
}
...
Gfx *sight_draw_zoom(Gfx *gdl, bool sighton, f32 crossx, f32 crossy)
{
	...
	gdl = sight_draw_default(gdl, sighton, crossx, crossy);
...
Gfx *sight_draw(Gfx *gdl, bool sighton, s32 sight)
{
	...
	switch (sight) {
	case SIGHT_DEFAULT:
		gdl = sight_draw_default(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	case SIGHT_CLASSIC:
		gdl = sight_draw_classic(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	case SIGHT_2:
		gdl = sight_draw_type2(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	case SIGHT_3:
		gdl = sight_draw_default(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	case SIGHT_SKEDAR:
		gdl = sight_draw_skedar(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	case SIGHT_ZOOM:
		gdl = sight_draw_zoom(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	case SIGHT_MAIAN:
		gdl = sight_draw_maian(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	default:
		gdl = sight_draw_default(gdl, sighton && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex), crossx, crossy);
		break;
	...
	if (sight != SIGHT_NONE && options_get_sight_on_screen(g_Vars.currentplayerstats->mpindex)) {
		if ((options_get_always_show_target(g_Vars.currentplayerstats->mpindex) && !sighton)
			...
			gdl = sight_draw_target(gdl, crossx, crossy);
		}
	}
