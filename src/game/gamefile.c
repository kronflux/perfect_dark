...
void gamefile_load_defaults(struct gamefile *file)
{
	...
	if (g_PlayerExtCfg[0].extcontrols) {
		options_set_control_mode(player1, CONTROLMODE_PC);
	}
	if (g_PlayerExtCfg[1].extcontrols) {
		options_set_control_mode(player2, CONTROLMODE_PC);
	}
#endif
...
#else
	pak_set_bitflag(GAMEFILEFLAG_P1_FORWARDPITCH, file->flags, true);
#endif
...
#else
	pak_set_bitflag(GAMEFILEFLAG_P1_LOOKAHEAD, file->flags, false);
#endif
...
#else
	pak_set_bitflag(GAMEFILEFLAG_P2_FORWARDPITCH, file->flags, true);
#endif
...
#else
	pak_set_bitflag(GAMEFILEFLAG_P2_LOOKAHEAD, file->flags, false);
#endif
...
s32 gamefile_load(s32 device)
{
	...
	if (device >= 0) {
		...
		if (ret == 0) {
			...
			// override with PC controls if enabled in the config
			if (g_PlayerExtCfg[0].extcontrols) {
				options_set_control_mode(p1index, CONTROLMODE_PC);
			}
			if (g_PlayerExtCfg[1].extcontrols) {
				options_set_control_mode(p2index, CONTROLMODE_PC);
			}
#endif
...
s32 gamefile_save(s32 device, s32 fileid, u16 deviceserial)
{
	...
	if (device >= 0) {
		...
		s32 controlmode = options_get_control_mode(p1index);
		savebuffer_or(&buffer, ((controlmode == CONTROLMODE_PC) ? CONTROLMODE_11 : controlmode), 3);
		controlmode = options_get_control_mode(p2index);
		savebuffer_or(&buffer, ((controlmode == CONTROLMODE_PC) ? CONTROLMODE_11 : controlmode), 3);
#endif
