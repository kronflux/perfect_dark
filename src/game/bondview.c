...
Gfx *bview_draw_static(Gfx *gdl, u32 arg1, s32 arg2)
{
	...
	for (y = viewtop; y < viewtop + viewheight; y++) {
		gdl = bview_copy_pixels(gdl, fb2, rngRandom() % 240, 5, y, 1.0f, viewleft, viewwidth);
	}
#else
	gDPSetCombineLERP(gdl++,
...
Gfx *bview_draw_eyespy_metrics(Gfx *gdl)
{
	...
	if (options_get_control_mode(g_Vars.currentplayerstats->mpindex) == CONTROLMODE_PC) {
		umask = U_CBUTTONS;
		dmask = D_CBUTTONS;
		lmask = L_CBUTTONS;
		rmask = R_CBUTTONS;
...

	{
		...
		u32 buttonsdown = joy_get_buttons(contpadnum, 0xffffffff); \
		u32 buttonsthisframe = joy_get_buttons_pressed_this_frame(contpadnum, 0xffffffff);
