...
struct mpconfigfull *challenge_load_config(s32 confignum, u8 *buffer, s32 len)
{
	...
	// Load mpconfigs
#ifdef PLATFORM_N64
	mpconfig = dma_exec_with_auto_align(buffer, (BTYPE)&_mpconfigsSegmentRomStart[confignum], sizeof(struct mpconfig));
#else
	mpconfig = dma_exec_with_auto_align(buffer, (BTYPE)REF_SEG _mpconfigsSegmentRomStart + confignum * sizeof(struct mpconfig), sizeof(struct mpconfig));
#endif
