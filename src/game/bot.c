...
void bot_spawn(struct chrdata *chr, u8 respawning)
{
	...
	if (aibot) {
		...
		if (g_Vars.normmplayerisrunning
			...
			botinv_give_single_weapon(chr, mpweapon->weaponnum);
			...
			if (ammotype) {
				...
				botact_give_ammo_by_type(aibot, ammotype, startammo);
			}
			botinv_switch_to_weapon(chr, mpweapon->weaponnum, FUNC_PRIMARY);
		}
#endif
	}
}
...
#define HASENOUGHPRI(aibot, weaponnum, goal) (g_AibotWeaponPreferences[weaponnum].haspriammogoal && botact_get_ammo_quantity_by_weapon(aibot, weaponnum, FUNC_PRIMARY, true) >= (goal))
#define HASENOUGHSEC(aibot, weaponnum, goal) (g_AibotWeaponPreferences[weaponnum].hassecammogoal && botact_get_ammo_quantity_by_weapon(aibot, weaponnum, FUNC_SECONDARY, true) >= (goal))
