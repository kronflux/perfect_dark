 ...
 * using the same process as shot_calculate_hits, except with no side effects and cheap = true.
...
bool shotTestLos(struct coord *gunpos2d, struct coord *gundir2d, struct coord *gunpos3d, struct coord *gundir3d, struct coord *endpos3d)
{
	...
	for (i = 0; rooms[i] != -1; i++) {
		if (bg_test_hit_in_room(&shotdata.gunpos3d, endpos3d, rooms[i], &hitthing)) {
	...
	while (propptr >= g_Vars.onscreenprops) {
		...
		if (prop) {
			if (prop->type == PROPTYPE_CHR
					|| (prop->type == PROPTYPE_PLAYER && prop->chr && (g_Vars.in_cutscene || playermgr_get_player_num_by_prop(prop) != g_Vars.currentplayernum
						|| g_Vars.currentplayer->eyespy && g_Vars.currentplayer->eyespy->mode == EYESPYMODE_CAMSPY))) {
				chr_test_hit(prop, &shotdata, false, true);
