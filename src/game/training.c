...
void *fr_load_rom_data(u32 len)
{
	...
	if (g_FrRomData) {
		return dma_exec_with_auto_align(g_FrRomData, (romptr_t) REF_SEG _firingrangeSegmentRomStart, len);
	}
...
void fr_end_session(bool hidetargets)
{
	...
	u32 stack1;
#ifdef AVOID_UB
	RoomNum rooms2[11]; // prevent bg_room_get_neighbours from writing out of bounds
#else
	RoomNum rooms2[10];
#endif
	u32 stack2;
