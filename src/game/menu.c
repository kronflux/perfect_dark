...
char *menu_resolve_text(uintptr_t thing, void *dialogoritem)
{
	...
	// Text ID
	if (thing < 0x5a00) {
		return lang_get((uintptr_t)thing);
	}
	...
	for (col = 0; col < dialog->numcols; col++, curx += colwidth) {
		...
		for (row = 0; row < menu->cols[colindex].numrows; row++) {
			...
			if (my > cury && my < cury + rowheight &&
					item != dialog->focuseditem && menu_is_item_focusable(item, dialog, 0)) {
				dialog->focuseditem = item;
				return true;
			}
			cury += rowheight;
...
s32 dialog_change_item_focus(struct menudialog *dialog, struct menuinputs *inputs)
{
	...
	if (inputs->updown != 0) {
		dialog_change_item_focus_vertically(dialog, inputs->updown);
	}

	if (inputs->leftright != 0) {
		swipedir = dialog_change_item_focus_horizontally(dialog, inputs->leftright);
	}
		...
		if (menumodel->newparams == menumodel->curparams) {
			...
			if (menumodel->loaddelay == 0) {
				if (MENUMODELPARAMS_GET_FILENUM(menumodel->newparams) == 0xffff || MENUMODELPARAMS_HAS_MASTER_HEADBODY(menumodel->newparams)) {
					...
					totalfilelen = file_get_inflated_size(bodyfilenum, LOADTYPE_MODEL);
					...
					if (g_HeadsAndBodies[bodynum].unk00_01) {
						...
						totalfilelen += ALIGN64(file_get_inflated_size(headfilenum, LOADTYPE_MODEL));
					}
					...
					totalfilelen = ALIGN64(file_get_inflated_size(menumodel->newparams, LOADTYPE_MODEL)) + 0x4000;
					if (1);
...
#ifdef AVOID_UB
		// during the credits cam_get_look_at() can return NULL
		if (cam_get_look_at())
#endif
...
void menu_reset(void)
{
	...
	if (!g_Vars.mplayerisrunning) {
		...
		for (i = 0; i < max; i++) {
#ifdef PLATFORM_64BIT
			menu_reset_model(&g_Menus[i].menumodel, IS4MB() ? 0xb400 : 0x38400, true); // 50% more
#else
		...
		if (IS8MB()) {
#ifdef PLATFORM_64BIT
			menu_reset_model(&g_MenuData.hudpiece, 0x12c00, true); // 50% more
#else
...
void dialog_tick(struct menudialog *dialog, struct menuinputs *inputs, u32 tickflags)
{
	...
	if (usedefaultbehaviour && (tickflags & MENUTICKFLAG_DIALOGISCURRENT) && !dialog->dimmed) {
		...
		if (layer->numsiblings <= 1) {
			...
			dialog_change_item_focus(dialog, inputs);
			...
			struct menuitem *prevfocuseditem = dialog->focuseditem;
			s32 swipedir = dialog_change_item_focus(dialog, inputs);
...
void menu_process_input(void)
{
	...
	if (g_Menus[g_MpPlayerNum].curdialog) {
		...
		for (i = 0; i < numcontpads; i++) {
			...
			u32 buttons = joy_get_buttons(contpadnums[i], 0xffffffff);
			u32 buttonsnow = joy_get_buttons_pressed_this_frame(contpadnums[i], 0xffffffff);
	...
	case MENUBG_BLUR:
		{
			...
			gdl = menugfx_render_bg_blur(gdl, 0xffffff00 | alpha >> 1, -BLUR_OFS, -BLUR_OFS);
			gdl = menugfx_render_bg_blur(gdl, 0xffffff00 | alpha >> 1, BLUR_OFS, BLUR_OFS);
		}
		break;
	case MENUBG_BLACK:
