...
void lang_reset(s32 stagenum)
{
	...
	g_LangBanks[LANGBANK_GUN] = file_load_to_new(lang_get_file_id(LANGBANK_GUN), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);

	g_LoadType = LOADTYPE_LANG;
	g_LangBanks[LANGBANK_MPMENU] = file_load_to_new(lang_get_file_id(LANGBANK_MPMENU), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);

	g_LoadType = LOADTYPE_LANG;
	g_LangBanks[LANGBANK_PROPOBJ] = file_load_to_new(lang_get_file_id(LANGBANK_PROPOBJ), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);

	g_LoadType = LOADTYPE_LANG;
	g_LangBanks[LANGBANK_MPWEAPONS] = file_load_to_new(lang_get_file_id(LANGBANK_MPWEAPONS), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);

	g_LoadType = LOADTYPE_LANG;
	g_LangBanks[LANGBANK_OPTIONS] = file_load_to_new(lang_get_file_id(LANGBANK_OPTIONS), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);

	g_LoadType = LOADTYPE_LANG;
	g_LangBanks[LANGBANK_MISC] = file_load_to_new(lang_get_file_id(LANGBANK_MISC), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);

	if (stagenum == STAGE_CREDITS) {
		g_LoadType = LOADTYPE_LANG;
		g_LangBanks[LANGBANK_TITLE] = file_load_to_new(lang_get_file_id(LANGBANK_TITLE), FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);
	}
#endif
}
