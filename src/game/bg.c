...
void bg_reset(s32 stagenum)
{
...
#if PIRACYCHECKS
	{
		...
		if (actualvalue != expectedvalue) {
			...
			dma_exec((void *)(PHYS_TO_K0(0x1000) + (rngRandom() & 0x1fff8)), rngRandom() & 0x1fffe, 0x40);
		}
	}
#endif
...
{
	gdl = bg_scissor_within_viewport(gdl, viewleft, viewtop, ceiltoint(viewright), ceiltoint(viewbottom));
...
void bg_load_room(s32 roomnum)
{
	...
	if (allocation != NULL) {
		...
		tex_copy_gdls((void *) gfxblocks[0], (void *) (allocation + alloclen - (gfxblocks[numgdls] - gfxblocks[0])), (uintptr_t) (gfxblocks[numgdls] - gfxblocks[0]));
...
void bgCalculateGlaresForVisibleRooms(void)
{
	...
	if (!g_Vars.mplayerisrunning) {
		for (i = 1; i < g_Vars.roomcount; i++) {
			if (g_Rooms[i].flags & ROOMFLAG_ONSCREEN) {
				artifacts_calculate_glares_for_room(i);
				if (g_NumRoomsWithGlares < 100) {
