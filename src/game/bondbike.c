...
void bbike_handle_activate(void)
{
	if (g_Vars.currentplayer->bondvehiclemode == VEHICLEMODE_RUNNING
		...
		bbike_try_dismount_angle(1.5705462694168f, sidedist);  // 90  - left
		bbike_try_dismount_angle(4.7116389274597f, sidedist);  // 270 - right
		bbike_try_dismount_angle(0.7852731347084f, diagdist);  // 45  - front left
		bbike_try_dismount_angle(5.4969120025635f, diagdist);  // 315 - front right
		bbike_try_dismount_angle(2.3558194637299f, diagdist);  // 135 - back left
		bbike_try_dismount_angle(3.9263656139374f, diagdist);  // 225 - back right
		bbike_try_dismount_angle(0, frontdist);                // 0   - front
		bbike_try_dismount_angle(3.1410925388336f, frontdist); // 180 - back
...
void bbike_apply_move_data(struct movedata *data)
{
	...
	f32 value1;
	f32 tmp;
	s32 contmode = options_get_control_mode(g_Vars.currentplayerstats->mpindex);
	...
	if ((contmode == CONTROLMODE_12
		...
		data->digitalstepleft = joy_count_buttons_on_specific_samples(0, contnum, lmask);
		data->digitalstepright = joy_count_buttons_on_specific_samples(0, contnum, rmask);
	}
