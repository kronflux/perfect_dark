		...
		for (i = 0; i < MAX_PLAYERS; i++) {
			...
			if (g_Menus[g_MpPlayerNum].curdialog) {
				...
				if (g_MenuData.root == MENUROOT_MPSETUP || g_MenuData.root == MENUROOT_4MBMAINMENU) {
					...
					bool canjoin;
					u32 buttons = joy_get_buttons_pressed_this_frame(i, 0xffffffff);
				...
				if (g_MenuData.root == MENUROOT_MPENDSCREEN) {
					u32 buttons2 = joy_get_buttons_pressed_this_frame(g_PlayerConfigsArray[i].contpad1, 0xffffffff);
	...
	if ((g_MenuData.isdialogopen || g_MenuData.prevmenuroot != -1) && isdialogopen == false) {
		...
		if (g_MenuData.prevmenuroot != -1) {
			if (g_MenuData.prevmenuroot == -5) {
				...
				menu_push_root_dialog(g_MenuData.prevmenudialog, g_MenuData.prevmenuroot);
				isdialogopen = true;
