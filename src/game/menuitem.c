...
Gfx *menuitem_keyboard_render(Gfx *gdl, struct menurendercontext *context)
{
	...
	gdl = menugfx_draw_filled_rect(gdl, context->x + 4, context->y + 5 * 11 + 13,
			context->x + 5, context->y + 6 * 11 + 14, 0x00ffff7f, 0x00ffff7f);
	gdl = menugfx_draw_filled_rect(gdl, context->x + 10 * 12 + 4, context->y + 5 * 11 + 13,
		...
		for (row = 0; row < MENU_KEYBOARD_ROWS; row++) {
			...
			if (row == 4) {
				...
				if (col == 0) {
					...
					++y;
					text_measure(&textheight, &textwidth, kbtext, g_CharsHandelGothicXs, g_FontHandelGothicXs, 0);
					x = context->x + (context->width - textwidth) / 2;
					gdl = text_render_projected(gdl, &x, &y, kbtext, g_CharsHandelGothicXs, g_FontHandelGothicXs, textcolour, context->width, context->height, 0, 0);
				}
#endif
			} else {
				// Alpha-numeric cell
...
Gfx *menuitem_carousel_render(Gfx *gdl, struct menurendercontext *context)
{
	...
	// Left arrow
	gdl = menugfx_draw_carousel_chevron(gdl, context->x + chevronOffset, context->y + context->height / 2, 8, 1, -1, colour);

	// Right arrow
	gdl = menugfx_draw_carousel_chevron(gdl, context->x + context->width - chevronOffset, context->y + context->height / 2, 8, 3, -1, colour);
...
#endif
		gdl = text0f153628(gdl);
		gdl = text_render_projected(gdl, &x, &y,
