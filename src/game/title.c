...
#ifdef PLATFORM_N64
#define ELEM_TEXT lang_get(elem->textid)
#else
#define ELEM_TEXT (char *)(elem->textptr ? elem->textptr : lang_get(elem->textid))
#endif
#if VERSION == VERSION_JPN_FINAL
				u32 stack;
				...
				if (elem->type == LEGALELEMENTTYPE_WHITETEXTLG || elem->type == LEGALELEMENTTYPE_WHITETEXTSM) {
					...
					gdl = text_render_projected(gdl, &x, &y, ELEM_TEXT, font1, font2, colour, vi_get_width(), vi_get_height(), 0, 0);

					var8007fad0 = 1;
					var80080108jf = 1;
				} else {
					gdl = text_render_projected(gdl, &x, &y, ELEM_TEXT, font1, font2, (colour & 0xffffff00) | ((colour & 0xff) * 2 / 3), vi_get_width(), vi_get_height(), 0, 0);
...
void title_tick_pd_logo(void)
{
	...
	if (joy_get_buttons_pressed_this_frame(0, 0xffffffff)) {
...
Gfx *title_render_pd_logo(Gfx *gdl)
{
...
#endif
	{
		...
		gdl = title_render_pd_logo_model(gdl, model, var80062804, g_PdLogoFrac, 240, 1.0f, &sp270, gfxAllocateVertices(numvertices), gfxAllocateColours(numcolours));
	}
...
void title_tick_rare_presents(void)
{
	...
	if (g_TitleTimer > TICKS(300)) {
		title_set_next_mode(TITLEMODE_PDLOGO);
	} else if (joy_get_buttons_pressed_this_frame(0, 0xffffffff)) {
		title_set_next_mode(TITLEMODE_SKIP);
	}
}
