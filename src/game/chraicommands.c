...
bool ai_give_object_to_chr(void)
{
	...
	if (obj && obj->prop && chr && chr->prop) {
		if (chr->prop->type == PROPTYPE_PLAYER) {
			...
			something = prop_pickup_by_player(obj->prop, 1);
...
bool ai00df(void)
{
	...
	if (tag) {
		...
		if (cmdindex >= 0) {
			struct warpparams *params = (struct warpparams *) setup_get_cmd_by_index(cmdindex + tag->cmdoffset);
...
bool ai_do_preset_animation(void)
{
	...
	if (cmd[2] == 255) {
		chr_try_start_anim(g_Vars.chrdata, anims[7 + (rngRandom() % 8)], 0, -1, 0, 15, 0.5);
	} else if (cmd[2] == 254) {
		...
		chr_try_start_anim(g_Vars.chrdata, anims[3 + (rngRandom() & 1)], 0, -1, 0, 15, 0.5);
	} else {
