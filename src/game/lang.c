...
#define TMUL 8
	...
	if (!multibyte && freeindexsingle >= 0) {
		...
		dma_exec(&g_JpnCharCachePixels[freeindexsingle * 8], (romptr_t) REF_SEG _fontjpnsingleSegmentRomStart + (codepoint >> SHIFTAMOUNT) * 0x60, 0x60);
	...
	if (multibyte && freeindexmulti >= 0) {
		...
		dma_exec(&g_JpnCharCachePixels[freeindexmulti * 8], (romptr_t) REF_SEG _fontjpnmultiSegmentRomStart + ((codepoint & 0x1fff) >> SHIFTAMOUNT) * 0x80, 0x80);
...
{
#if VERSION >= VERSION_PAL_BETA
	s32 len = file_get_inflated_size(lang_get_file_id(bank), LOADTYPE_LANG);
	...
	g_LoadType = LOADTYPE_LANG;
	g_LangBanks[bank] = file_load_to_new(file_id, FILELOADMETHOD_DEFAULT, LOADTYPE_LANG);
#endif
}
