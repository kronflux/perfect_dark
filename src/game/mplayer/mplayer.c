...
void mp_apply_weapon_set(void)
{
	...
	if (g_MpWeaponSetNum >= 0 && g_MpWeaponSetNum < ARRAYCOUNT(g_MpWeaponSets)) {
		...
		for (i = 0; i < ARRAYCOUNT(g_MpSetup.weapons); i++) {
			mp_set_weapon_slot(i, rngRandom() % numoptions);
		}
#else
		mpSetRandomWeapons(randomweapons);
		for (i = 0; i < ARRAYCOUNT(g_MpSetup.weapons); i++) {
			mp_set_weapon_slot(i, randomweapons[rngRandom() % g_MpWeaponRandomFilterNum]);
		}
#endif
		...
		for (i = 0; i < 5; i++) {
			mp_set_weapon_slot(i, rngRandom() % numoptions + 1);
		}
#else
		mpSetRandomWeapons(randomweapons);
		for (i = 0; i < 5; i++) {
...
s32 mp_choose_track(void)
{
	...
	if (mp_get_using_multiple_tunes()) {
		...
		if (numselected == 0) {
			do {
				tracknum = mp_get_track_num_at_slot_index(rngRandom() % numunlocked);
...
{
	s32 dest = mp_get_slot_for_new_bot();
...
void mpplayerfile_save_wad(s32 playernum, struct savebuffer *buffer)
{
	...
	savebuffer_or(buffer, ((controlmode == CONTROLMODE_PC) ? CONTROLMODE_11 : controlmode), 2);
#endif
...
void mpsetupfile_load_wad(struct savebuffer *buffer, u8 version)
{
	s32 i;
	s32 j;
	...
	scenario_read_save(buffer, version);

	if (version > 0) {
		g_MpSetup.options = savebuffer_read_bits(buffer, 32);
	}
	else {
	...
	if (version > 0) {
		u64 wpnRndPacked = savebuffer_read_bits(buffer, 64);
		unpackWeaponSetRandomFilters(wpnRndPacked);
	}

	g_MpWeaponSetNum = savebuffer_read_bits(buffer, 8);
...
void mpsetupfile_save_wad(struct savebuffer *buffer)
{
	...
	savebuffer_or(buffer, g_MpSetup.options, 32);
	...
	savebuffer_or(buffer, wpnRndPacked, 64);
	savebuffer_or(buffer, g_MpWeaponSetNum, 8);
