...
void setup_create_door(struct doorobj *door, s32 cmdindex)
{
	...
	if (pad.room > 0) {
		...
		mtx4_load_x_rotation(1.5705462694168f, &finalmtx);
		mtx4_load_z_rotation(1.5705462694168f, &zrotmtx);
...
void setup_load_files(s32 stagenum)
{
	...
	if (stagenum < STAGE_TITLE) {
		...
		g_GeCreditsData = (u8 *)file_load_to_new(filenum, FILELOADMETHOD_DEFAULT, LOADTYPE_SETUP);
		...
		g_StageSetup.padfiledata = file_load_to_new(g_Stages[g_StageIndex].padsfileid, FILELOADMETHOD_DEFAULT, LOADTYPE_PADS);
		...
		if (obj->type == OBJTYPE_LIFT) {
			...
			for (int i = 0; i < ARRAYCOUNT(lift->doors); i++) {
				if (lift->doors[i]) {
					...
					struct doorobj *door = (struct doorobj *)setup_get_cmd_by_index(doorindex);
					// we use this 'extra1' field here to mark the door, since its not used anywhere else
					door->extra1 = 1;

					if (door->sibling) {
						s32 siblingidx = *(s32 *) &door->sibling + doorindex;
						struct doorobj *sibling = (struct doorobj *) setup_get_cmd_by_index(siblingidx);
						sibling->extra1 = 1;
					}
				}
			}
			...
			while (obj->type != OBJTYPE_END) {
				...
				case OBJTYPE_ESCASTEP:
					if (withobjs && (obj->flags2 & diffflag) == 0) {
						...
						if (obj->flags & OBJFLAG_ESCSTEP_ZALIGNED) {
							step->frame = escstepy;
							escstepy += 40;
							mtx4_load_y_rotation(4.7116389274597f, (Mtxf *) &sp1a8);
				...
				case OBJTYPE_TRUCK:
					if (withobjs && (obj->flags2 & diffflag) == 0) {
						...
						truck->turnrot60 = 0;
						truck->roty = 0;
						truck->ailist = ailist_find_by_id((uintptr_t)truck->ailist);
						truck->aioffset = 0;
				...
				case OBJTYPE_HOVERCAR:
					if (withhovercars && withobjs && (obj->flags2 & diffflag) == 0) {
						...
						car->rotx = 0;
						car->speedtime60 = -1;
						car->ailist = ailist_find_by_id((uintptr_t)car->ailist);
						car->aioffset = 0;
						car->aireturnlist = -1;
						car->path = NULL;
						car->nextstep = 0;
				...
				case OBJTYPE_CHOPPER:
					if (withobjs && (obj->flags2 & diffflag) == 0) {
						...
						chopper->ailist = ailist_find_by_id((uintptr_t)chopper->ailist);
						chopper->aioffset = 0;
				...
				case OBJTYPE_HELI:
					if (withobjs && (obj->flags2 & diffflag) == 0) {
						...
						heli->ailist = ailist_find_by_id((uintptr_t)heli->ailist);
						heli->aioffset = 0;
						heli->aireturnlist = -1;
						heli->path = NULL;
						heli->nextstep = 0;
