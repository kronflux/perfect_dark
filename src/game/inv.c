...
s32 inv_give_weapons_by_prop(struct prop *prop)
{
	...
	if (prop->type == PROPTYPE_WEAPON) {
		...
		if (obj->type == OBJTYPE_WEAPON) {
			...
			if (!g_Vars.normmplayerisrunning && cheat_is_active(CHEAT_DUALWIELDALLGUNS)) {
				if (inv_has_single_weapon_exc_all_guns(weaponnum) && !inv_has_double_weapon_exc_all_guns(weaponnum, weaponnum)) {
