...
char *filemgr_menu_text_fail_reason(struct menuitem *item)
{
	...
	return lang_get(reasons[g_Menus[g_MpPlayerNum].fm.errnum]);
}
...
void filemgr_push_error_dialog(u16 errnum)
{
 ...
 */
uintptr_t filemgr_get_device_name_or_start_index(s32 listnum, s32 operation, s32 optionindex)
{
	u16 names[] = {
		L_OPTIONS_111, // "Game Pak"
		L_OPTIONS_112, // "Controller Pak 1"
		...
		if (g_FileLists[listnum]->devicestartindexes[i] != -1) {
			if (remaining == 0) {
				if (operation == MENUOP_GETOPTGROUPTEXT) {
					return (uintptr_t)lang_get(names[i]);
				}
...
bool filemgr_attempt_operation(s32 device, bool closeonsuccess)
{
	...
	case FILEOP_WRITE_MPPLAYER:
		...
		break;
	case FILEOP_LOAD_GAME:
		errnum = gamefile_load(device);
		break;
	case FILEOP_LOAD_MPPLAYER:
		errnum = mpplayerfile_load(
	...
	case FILEOP_READ_MPPLAYER:
#if VERSION >= VERSION_NTSC_1_0
		errnum = pak_read_body_at_guid(device, g_Menus[g_MpPlayerNum].fm.fileid, g_Menus[g_MpPlayerNum].fm.unke44, 0);
#else
		errnum = pak_read_body_at_guid(device, g_Menus[g_MpPlayerNum].fm.fileid, g_Menus[g_MpPlayerNum].fm.unke44,
 ...
 */
bool filemgr_save_or_load(struct fileguid *guid, s32 fileop, uintptr_t playernum)
{
	s32 device;
...
void func0f1097d0(s32 device)
{
	...
	if (g_FileLists[0]) {
		...
		if (thing) {
			filemgr_save_or_load(&g_FilemgrFileToCopy, g_Menus[g_MpPlayerNum].fm.filetypeplusone + 103, (uintptr_t) thing);
		} else {
...
MenuItemHandlerResult filemgr_choose_agent_list_menu_handler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_25:
		...
		if (g_FileAutoSelect >= 0 && g_FileLists[0] && g_FileLists[0]->numfiles > 0) {
			...
			g_FileAutoSelect = -1;
			filemgr_choose_agent_list_menu_handler(MENUOP_SET, item, data);
			data->list.value = tmp;
		}
#endif
		break;
