...
static inline s32 rzipInflate1172(z_stream *strm, u8 *src, void *dst)
{
	...
	do {
		...
		if (inflate(strm, Z_FINISH) == Z_STREAM_ERROR) {
			rmon_printf("rzipInflate1172: Z_STREAM_ERROR\n");
			return 0;
		}
...
static inline s32 rzipInflate1173(z_stream *strm, u8 *src, void *dst, u32 dstLen)
{
	...
	if (inflate(strm, Z_SYNC_FLUSH) == Z_STREAM_ERROR) {
		rmon_printf("rzipInflate1173: Z_STREAM_ERROR\n");
		return 0;
	}
...
s32 rzip_inflate(void *srcp, void *dst, void *scratch)
{
	s32 ret = 0;
	u8 *src = srcp;
	z_stream strm = { 0 };
	...
	if (ret != Z_OK) {
		rmon_printf("rzip_inflate: inflateInit2 failed: %d\n", ret);
		return 0;
	}
	...
	if (rzip_is_1173(src)) {
		...
		rmon_printf("rzip_inflate: input not in any known rare zip format\n");
		ret = 0;
	}
