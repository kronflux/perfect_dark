...
const char var70054080[] = "joy_reset\n";
const char var7005408c[] = "joy_reset: doing nothing\n";
...
u32 joy_get_buttons_on_sample(s32 samplenum, s8 contpadnum, u32 mask)
{
	u32 button;
...
u32 joy_get_buttons_pressed_on_sample(s32 samplenum, s8 contpadnum, u32 mask)
{
	u32 button1;
	u32 button2;
 ...
 */
s32 joy_count_buttons_on_specific_samples(u32 *checksamples, s8 contpadnum, u32 mask)
{
	s32 count = 0;
	s32 index = 0;
	s32 i;
...
u32 joy_get_buttons(s8 contpadnum, u32 mask)
{
	if (g_JoyDataPtr->unk200 < 0 && (g_JoyConnectedControllers >> contpadnum & 1) == 0) {
		g_JoyBadReadsButtons[contpadnum]++;
		return 0;
...
u32 joy_get_buttons_pressed_this_frame(s8 contpadnum, u32 mask)
{
	if (g_JoyDataPtr->unk200 < 0 && (g_JoyConnectedControllers >> contpadnum & 1) == 0) {
		g_JoyBadReadsButtonsPressed[contpadnum]++;
		return 0;
...
#if VERSION < VERSION_NTSC_1_0
u32 joy_get_buttons_released_this_frame(s8 contpadnum, u32 mask)
{
	if (g_JoyDataPtr->unk200 < 0 && (g_JoyConnectedControllers >> contpadnum & 1) == 0) {
		g_JoyBadReadsButtonsPressed[contpadnum]++;
		return 0;
