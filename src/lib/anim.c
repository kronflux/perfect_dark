...
void anims_init(void)
{
	...
	dma_exec(ptr, (romptr_t) REF_SEG _animationsTableRomStart, tablelen);
...
#ifndef PLATFORM_N64
	g_AnimReplacements    = memp_alloc(ALIGN64(g_NumAnimations * sizeof(u8 *)), MEMPOOL_PERMANENT);
...
u8 *anim_dma(u8 *dst, u32 segoffset, u32 len)
{
	...
	return dma_exec_with_auto_align(dst, (romptr_t) REF_SEG _animationsSegmentRomStart + segoffset, len);
}
