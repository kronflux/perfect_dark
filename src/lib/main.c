...
#define BUTTON_MASK (U_CBUTTONS | D_CBUTTONS | L_CBUTTONS | R_CBUTTONS)
...

	{
		...
		if (g_DoBootPakMenu) {
			dma_exec(fb, (romptr_t) REF_SEG _accessingpakSegmentRomStart, REF_SEG _accessingpakSegmentRomEnd - REF_SEG _accessingpakSegmentRomStart);
		} else {
			dma_exec(fb, (romptr_t) REF_SEG _copyrightSegmentRomStart, REF_SEG _copyrightSegmentRomEnd - REF_SEG _copyrightSegmentRomStart);
		}
		...
		while (j < 6) {
			...
			if (*(s16 *) receivedmsg == OS_SC_RETRACE_MSG) {
				vi_update_mode();
				rdp_create_task(var8005dcc8, var8005dcc8 + ARRAYCOUNT(var8005dcc8), 0, (uintptr_t) &scdonemsg);
				j++;
			}
		}
	}
	...
	if (g_MainChangeToStageNum < 0 && g_MainNumGfxTasks < NUM_GFXTASKS) {
		...
		if (g_MainGameLogicEnabled) {
			...
			if (g_MainIsDebugMenuOpen || joy_get_buttons(0, U_CBUTTONS | D_CBUTTONS) == (U_CBUTTONS | D_CBUTTONS)) {
				g_MainIsDebugMenuOpen = debug_process_input(joy_get_stick_x(0), joy_get_stick_y(0), joy_get_buttons(0, 0xffffffff), joy_get_buttons_pressed_this_frame(0, 0xffffffff));
